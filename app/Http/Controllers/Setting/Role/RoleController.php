...
class RoleController extends Controller
{
            ...
            [
                'tableStruct' => [
                    'datatable_1' => [
                        ...
                        $this->makeColumn('name:users|label:Jumlah User|className:text-center|width:200px|sortable:false'),
                        $this->makeColumn('name:permissions|label:Jumlah Akses|className:text-center|width:200px|sortable:false'),
    ...
    public function grid()
    {
        ...
        return \DataTables::of($records)
            ...
            ->addColumn(
                'users',
                function ($record) {
                    return Base::makeLabel($record->users()->count() . ' User', 'primary');
                }
            )
            ->addColumn(
                'permissions',
                function ($record) {
                    return Base::makeLabel($record->permissions()->count() . ' Permission', 'warning');
                }
            )
            ->editColumn(
                'updated_by',
    ...
    public function template()
    {
        ...
        $data = [];
        $data['modules'] = \Base::getModulesPerms();
        $data['actions'] = ['view', 'create', 'edit', 'delete', 'approve'];
        $data['colors'] = ['#3699FF', '#1BC5BD', '#8950FC', '#28C76F', '#FFA800'];
