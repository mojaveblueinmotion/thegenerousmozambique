...
trait RaidModel
{
    ...
    public function creatorName($with_position = false)
    {
        if ($this->updater) {
            return $this->updater->name . ($with_position && isset($this->updater->position->name) ? ' (' . $this->updater->position->name . ')' : '');
        }
        return isset($this->creator)
            ? $this->creator->name . ($with_position && isset($this->creator->position->name) ? ' (' . $this->creator->position->name . ')'
                : '') : '[System]';
    }
        ...
        return '<div data-order="' . ($this->updated_at ?: $this->created_at) . '" class="text-left make-td-py-0">
                    <small>
                        <div class="text-nowrap">
                            <i data-toggle="tooltip" title="' . \Str::title($this->creatorName(true)) . '"
