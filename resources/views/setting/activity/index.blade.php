    ...
    <div class="row">
        <div class="col-12 col-sm-6 col-xl-3 pb-2">
            <select class="form-control base-plugin--select2-ajax filter-control" data-post="module_name"
                ...
                @foreach (\Base::getModulesWithOutSubMenu() as $key => $val)
                    @if (\Str::startsWith($key, 'master.') || \Str::startsWith($key, 'setting.'))
                        @continue
                    @endif
        ...
        <div class="col-12 col-sm-6 col-xl-3 pb-2">
            <select class="form-control filter-control base-plugin--select2-ajax"
                data-url="{{ route('ajax.select-user', ['search' => 'all']) }}" data-post="created_by"
        ...
        <div class="col-12 col-sm-6 col-xl-3 pb-2">
            <div class="input-group">
                <input type="text" data-post="date_start"
                    class="form-control filter-control base-plugin--datepicker date-start" placeholder="{{ __('Mulai') }}"
                    data-options='@json([
                        'format' => 'dd/mm/yyyy',
                        'startDate' => '',
