		...
		<div class="card-body pt-15">
			...
			<div class="text-center mb-10">
				<div class="symbol symbol-60 symbol-circle symbol-xl-90 shadow">
                    <img src="{{ asset($user->image_path) }}" alt="Image">
					<i class="symbol-badge symbol-badge-bottom bg-success"></i>
				</div>
				<h4 class="font-weight-bold my-2">{{ $user->name }}</h4>
				<div class="text-muted mb-2">{{ $user->roles_imploded }}</div>
			...
			<a href="{{ route($routes.'.change-password') }}"
				class="btn btn-hover-light-primary font-weight-bold py-3 px-6 mb-2 text-center btn-block base-content--replace {{ ($tab == 'change-password') ? 'active' : '' }}">
				{{ __('Ubah Password') }}
			</a>
