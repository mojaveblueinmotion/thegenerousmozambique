...
@section('modal-body')
	...
	<div class="form-group row">
		...
		<div class="col-sm-10 parent-group">
			<input type="text" value="{{ $record->title }}" name="title" class="form-control" placeholder="{{ __('Judul') }}">
		</div>
	</div>
	<div class="form-group row">
		<label class="col-md-2 col-form-label">{{ __('Isi') }}</label>
		<div class="col-md-10 parent-group">
			<textarea name="description" class="base-plugin--summernote" placeholder="{{ __('Isi') }}">{{ $record->description }}</textarea>
		</div>
	</div>
	<div class="form-group row">
		<label class="col-sm-2 col-form-label">{{ __('Link') }}</label>
		<div class="col-sm-10 parent-group">
			<input type="text" name="link" class="form-control" value="{{ $record->link }}" placeholder="{{ __('Link') }}">
		</div>
	</div>
	<div class="form-group row">
